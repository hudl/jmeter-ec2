...
function runsetup() {
  ...
  if [ -z "$REMOTE_HOSTS" ] ; then
    ...
    (aws ec2 create-tags --resources ${attempted_instanceids[@]} --tags Key=Role,Value="RoleJmeter" --region $REGION)
    (aws ec2 create-tags --resources ${attempted_instanceids[@]} --tags Key=Group,Value="jmeter" --region $REGION)
    (aws ec2 create-tags --resources ${attempted_instanceids[@]} --tags Key=Environment,Value="test" --region $REGION)
    wait
    echo "complete"
    echo
